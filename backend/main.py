...
def get_context(query: str):
    ...
    cursor = conn.cursor()
    score_parts = [f"(CASE WHEN conteudo_limpo LIKE ? THEN 15 ELSE 0 END)" for _ in words]
    params = [f"%{w[:5]}%" for w in words]
    score_query = " + ".join(score_parts)
    sql = f"SELECT conteudo, ({score_query}) as relevance FROM documentos WHERE relevance >= 5 ORDER BY relevance DESC LIMIT 5"
...
async def ask(request: Request):
    ...
    async def event_generator():
        prompt = f"Use o CONTEXTO abaixo para responder à PERGUNTA do Grupo JB.\n\nCONTEXTO:\n{context}\n\nPERGUNTA: {question}"
        
        async with httpx.AsyncClient(timeout=None) as client:
            # Modelo 1B - Ultra Rápido
            async with client.stream("POST", "http://localhost:11434/api/generate", 
                                   json={"model": "llama3.2:1b", "prompt": prompt, "options": {"temperature": 0}}) as response:
